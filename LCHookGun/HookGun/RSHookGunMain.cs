...
namespace HookGun
{
    [BepInDependency(PluginInfos.MODUID3, BepInDependency.DependencyFlags.HardDependency)]
    [BepInDependency("FlipMods.ReservedItemSlotCore", BepInDependency.DependencyFlags.SoftDependency)]
    [BepInPlugin(PluginInfos.MODUID2, PluginInfos.MODNAME2, PluginInfos.MODVERSION)]
    public class RSHookGunMain : BaseUnityPlugin
    {

        private readonly Harmony harmony = new Harmony(PluginInfos.MODUID);
        ...
        void Awake()
        {
            ...
            mls = BepInEx.Logging.Logger.CreateLogSource(PluginInfos.MODUID);
        ...
        public class HookGunScript : GrabbableObject
        {
            ...
            public override void Update()
            {
                ...
                if (this.isHeld)
                {
                    ...
                    {
                        this.gameObject.transform.Find("HookMesh").gameObject.SetActive(true);
                    }
                    else
                    {
                        this.gameObject.transform.Find("HookMesh").gameObject.SetActive(false);
                    }
